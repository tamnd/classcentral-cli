// Package cli builds the cc command tree on top of the classcentral library.
package cli
...
type App struct {
	client *classcentral.Client
	cfg    classcentral.Config
...
func Root() *cobra.Command {
	app := &App{cfg: classcentral.DefaultConfig()}

	root := &cobra.Command{
		Use:   "cc",
		Short: "Search online courses on Class Central",
		Long: `cc reads Class Central — the aggregator for online courses from Coursera,
edX, Udemy, and hundreds of other providers — through its public HTML endpoints
with no API key or login required.

It returns structured records as table, JSON, JSONL, CSV, TSV, or URLs.

cc is an independent tool and is not affiliated with Class Central Inc.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = classcentral.NewClient(a.cfg)
	return nil
}
...
func mapFetchErr(err error) error {
	...
	if isBlocked(err) {
		_, _ = fmt.Fprintln(os.Stderr, "classcentral.com is blocking requests from this IP (Cloudflare challenge).")
		_, _ = fmt.Fprintln(os.Stderr, "Run cc from a residential or on-device connection.")
